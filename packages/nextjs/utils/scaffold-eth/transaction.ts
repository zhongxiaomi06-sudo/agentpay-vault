...
const transactionErrorText = (error: unknown) => {
  const value = error as { shortMessage?: string; details?: string; message?: string };
  return value?.shortMessage || value?.details || value?.message || String(error);
};

export const isExpectedTransactionError = (error: unknown) => {
  const lower = transactionErrorText(error).toLowerCase();
  return (
...
export const getFriendlyTransactionError = (error: unknown) => {
  ...
  if (
    ...
    return "钱包 MON 余额不足，无法支付金额或 Gas；请确认顶部余额并切换到已领取测试币的钱包";
