...
import { createPublicClient, createWalletClient, formatEther, http, keccak256, parseAbi, parseEther, parseSignature, toBytes } from "viem";
...
const deployerPk = readFileSync(new URL("../../hardhat/.env", import.meta.url), "utf8").match(/__RUNTIME_DEPLOYER_PRIVATE_KEY=(0x[0-9a-fA-F]+)/)[1];
...
await tx("openStream：预存 0.01 MON，费率 0.00001 MON/秒", wallet.writeContract({
  address: VAULT, abi: vaultAbi, functionName: "openStream", args: [main.address, rate], value: parseEther("0.01"),
}));
...
console.log("⏹");
await tx("服务商提取已归属收入 withdrawStream", wallet.writeContract({ address: VAULT, abi: vaultAbi, functionName: "withdrawStream", args: [streamId] }));

// ========== 第二幕：按次计量（x402 哲学） ==========
say("🎫 第二幕：按次计量 —— agent 零 gas 签名授权，服务商持签名上链记账");
await tx("createPlan：挂单 0.0001 MON/次", wallet.writeContract({ address: VAULT, abi: vaultAbi, functionName: "createPlan", args: [parseEther("0.0001")] }));
const planId = await pub.readContract({ address: VAULT, abi: vaultAbi, functionName: "planCount" });
await tx("subscribe：预付 3 次调用额度", wallet.writeContract({ address: VAULT, abi: vaultAbi, functionName: "subscribe", args: [planId, 3n], value: parseEther("0.0003") }));

for (let i = 0; i < 3; i++) {
  const callIndex = await pub.readContract({ address: VAULT, abi: vaultAbi, functionName: "callSeq", args: [planId, main.address] });
  const sig = await main.signTypedData({
    domain: { name: "AgentPayVault", version: "1", chainId: 10143, verifyingContract: VAULT },
    types: { MeterAuth: [{ name: "planId", type: "uint256" }, { name: "agent", type: "address" }, { name: "callIndex", type: "uint256" }] },
    primaryType: "MeterAuth",
    message: { planId, agent: main.address, callIndex },
  });
  const { r, s, yParity } = parseSignature(sig);
  await tx(`meter #${i + 1}：离线签名 → 链上记账（agent 未付 gas）`, wallet.writeContract({
    address: VAULT, abi: vaultAbi, functionName: "meter", args: [planId, main.address, callIndex, 27 + (yParity ?? 0), r, s],
  }));
}
const pendingBal = await pub.readContract({ address: VAULT, abi: vaultAbi, functionName: "pending", args: [main.address] });
ok(`服务商待提现累计: ${formatEther(pendingBal)} MON（pull over push）`);
await tx("服务商 withdrawProvider 提现", wallet.writeContract({ address: VAULT, abi: vaultAbi, functionName: "withdrawProvider" }));

// ========== 第三幕：乐观托管 ==========
say("🤝 第三幕：乐观托管 —— 先放款、争议惩罚，与 Monad 乐观并行同构");
const expectedHash = keccak256(toBytes("agent-task-result-v1"));
await tx("lockEscrow：锁定 0.003 MON（争议窗口 20 秒）", wallet.writeContract({
  address: VAULT, abi: vaultAbi, functionName: "lockEscrow",
...
await tx("deliver：服务商提交交付凭证哈希（争议窗口开启）", wallet.writeContract({ address: VAULT, abi: vaultAbi, functionName: "deliver", args: [escrowId, expectedHash] }));
console.log("  ⏳ 争议窗口 20 秒（买方此时可 release；装死则由服务商 claim；交付有异议可 dispute 50/50）…");
await sleep(21);
await tx("claim：窗口无争议，服务商乐观取款 100%", wallet.writeContract({ address: VAULT, abi: vaultAbi, functionName: "claim", args: [escrowId] }));
...
say("📊 压测结果");
for (const [bn, n] of [...byBlock.entries()].sort()) console.log(`  区块 ${bn}: ${n} 笔${n > 1 ? " ⚡ 同块并行打包" : ""}`);
